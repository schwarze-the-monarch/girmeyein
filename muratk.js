...
mongoose.connect('url buraya', {useNewUrlParser: true, useUnifiedTopology: true}); //Mongo bağlantı linki
    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
      if (!inviterData) {
        ...
        newInviter.save().then(x => {
          if (inviteChannel) inviteChannel.send(`<:Social_sariok:779697720263835658> ${member} katıldı! **Davet eden**: ${inviter.id == member.guild.id ? member.guild.name : inviter.tag} (**${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}** /<a:Social_sonsuz:779697721350422548>)`).catch(err => {});
        });
      } else {
        inviterData.fake++
      ...
      Database.findOne({ guildID: member.guild.id, userID: memberData.inviterID }, async (err, inviterData) => {
        if (!inviterData) {
         let newInviter = new Database({
          ...
          inviterData.save().then(x => {
            if (inviteChannel) inviteChannel.send(`<:Social_sariok:779697720263835658> ** ${member.user.tag} **ayrıldı! ${inviter.tag ? `**Davet eden**: ${inviter.id == member.guild.id ? member.guild.name : inviter.tag} **Kalan Davet Sayısı:** (**${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}** /<a:Social_sonsuz:779697721350422548>) ` : `Davetçi bulunamadı!`}`).catch(err => {});
          });
        };
      });
    } else {
      if (inviteChannel) inviteChannel.send(`<:Social_sariok:779697720263835658> ** ${member.user.tag} **ayrıldı! Davetçi bulunamadı!`).catch(err => {});
    };
  });
});
...
client.on("message", async message => {
  ...
  if (command === "eval" && message.author.id === botOwner) {
    ...
    try { 
      ...
      if(evaled.match(new RegExp(`${client.token}`, 'g'))) evaled.replace("token", "Ananın Tokenini Vereyimmi Şimdi ?").replace(client.token, "Ananın Tokenini Vereyimmi Şimdi ?");
      message.channel.send(`${evaled.replace(client.token, "Ananın Tokenini Vereyimmi Şimdi ?")}`, {code: "js", split: true});
    } catch(err) { message.channel.send(err, {code: "js", split: true}) };
  };

  if (command === "davetlerim" || command === "rank" || command === "invites") {
    let uye = message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.member;
    let embed = new MessageEmbed().setAuthor(uye.displayName, uye.user.displayAvatarURL({dynamic: true})).setColor(uye.displayHexColor);
    Database.findOne({guildID: message.guild.id, userID: uye.id}, (err, inviterData) => {
      if (!inviterData) {
        ...
        Database.find({guildID: message.guild.id, inviterID: uye.id}).sort().exec((err, inviterMembers) => {
          ...
          embed.setDescription(`<a:Social_setting:779697722814496808> **${inviterData.regular+inviterData.bonus}**  Toplam Davet Sayısı. \n\n<a:Social_setting:779697722814496808> **${inviterData.regular}**  Gerçek Davet Sayısı. \n\n<a:Social_setting:779697722814496808> **${inviterData.bonus}**  Bonus Davetler.(Takviye) \n\n<a:Social_setting:779697722814496808> **${inviterData.fake}**  Sahte Davet Sayısı. \n\n<a:Social_setting:779697722814496808> **${dailyInvites}**  Günlük Davet Sayısı. \n\n<:Social_sariok:779697720263835658> Davet ettiğiniz üyeler sunucudan çıktığı takdirde davet sayınızde düşüş yapılır.`);
          message.channel.send(embed);
        });
      };
    });
  ...
  if (command === "bonus") {
    if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("Rosy Olmalısın")
